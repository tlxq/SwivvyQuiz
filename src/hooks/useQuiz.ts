...
import { fetchTriviaQuestions, fetchTriviaCategories } from '@/services/triviaService';
import { decodeHtmlEntities } from '@/utils/triviaHelpers';
import type { TriviaQuestion, TriviaCategory } from '@/types/trivia';
import { TOTAL_QUESTIONS, TIMER_DURATION, BASE_POINTS, MAX_BONUS } from '@/constants/quiz';
...
export function useQuiz() {
  ...
  // Extract timer logic to its own hook for better DRY compliance
  const { timeLeft, barProgress } = useTimer({
  ...
  const submitAnswer = useCallback((choice: 'True' | 'False' | null) => {
  ...
  return {
    questions, categories, loading, loadingCategories, error,
    questionIndex, score, isCompleted, isAnswered, selectedAnswer,
    timeLeft, barProgress,
    loadCategories, startQuiz, submitAnswer, 
